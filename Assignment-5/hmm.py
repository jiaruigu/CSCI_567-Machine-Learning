  ...
  for t in reversed(range(N)):
    for i in range(S): 	
      if t == N-1:
        ...
        sum = 0
        for j in range(S):
          sum += beta[j][t+1]*A[i][j]*B[j][O[t+1]]
        beta[i][t] = sum
  return beta
  ...
  for t in range(N):
    for j in range(S):
      if t == 0:
        ...
        max_theta = 0
        for i in range(S):
          theta_j_t = theta[i][t-1]*A[i][j]*B[j][O[t]]
          if max_theta < theta_j_t:
            max_theta = theta_j_t
            back_arrows[j][t] = int(i)
        theta[j][t] = max_theta
  for t in reversed(range(N)):
    if t == N-1:
      path = [np.argmax(theta.T[-1])]
    else:
      path = [int(back_arrows[path[0]][t+1])] + path
  return path
