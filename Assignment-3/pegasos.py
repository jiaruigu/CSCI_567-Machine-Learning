...
def objective_function(X, y, w, lamb):
    """
    Inputs:
    - X: A 2 dimensional numpy array of data (number of samples x number of features)
    - y: A 1 dimensional numpy array of labels (length = number of samples )
    ...
    temp1 = lamb/2*np.linalg.norm(w)**2
    temp2 = 0
    for n in range(len(y)):
        temp2 += max(0,1-y[n]*np.matmul(w.T,X[n].T))
    obj_value = temp1+temp2/len(y)
    return obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        A_tp = []
        for i in range(k):
        	if np.matmul(w.T,Xtrain[A_t[i]].T)*ytrain[A_t[i]] < 1:
        		A_tp.append(A_t[i])
        temp = np.zeros(w.shape);
        for i in range(len(A_tp)):
        	temp += ytrain[A_tp[i]]*np.array([Xtrain[A_tp[i]]]).T
        w = (1-eta*lamb)*w+eta/k*temp
        w = w*min(1,1/lamb**0.5/np.linalg.norm(w))
    return w, train_obj
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    """
    # you need to fill in your solution here
    ypredict = (np.matmul(w.T, np.transpose(Xtest))[0] >= t).astype(int)
    ypredict[ypredict==0] = -1
    test_acc = 1 - (np.absolute(ypredict-ytest)).sum()/2/len(ytest)
    return test_acc
