...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output = np.zeros((X.shape[0], self.params['W'].shape[1]))
        for n in range(X.shape[0]):
            forward_output[n] = np.matmul(X[n],self.params['W'])+self.params['b']
    ...
    def backward(self, X, grad):
        ...
        ##########################################################################################################################
        gradient_W = np.zeros((X.shape[1], grad.shape[1]))
        gradient_b = np.zeros((1, grad.shape[1]))
        backward_output = np.zeros(X.shape)
        for n in range(X.shape[0]):
            gradient_W += np.matmul(np.transpose([X[n]]), [grad[n]])
            gradient_b += grad[n]
            backward_output[n] = np.matmul([grad[n]], self.params['W'].T)
        self.gradient['W'] = gradient_W
        self.gradient['b'] = gradient_b
...
class dropout:
    ...
    def backward(self, X, grad):
        ...
        for i in range(self.mask.shape[0]):
            backward_output[i] = self.mask[i]*grad[i]
        return backward_output
