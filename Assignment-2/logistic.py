...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    """
    X1 = np.copy(X)
    X1 = np.concatenate((np.transpose([[1 for n in range(N)]]), X1), axis=1)
    w1 = np.copy(w)
    w1 = np.concatenate(([0],w1), axis=0)

    for i in range(max_iterations):
        derivative = 0
        for n in range(N):
            derivative += (sigmoid(np.matmul(w1,np.transpose([X1[n]])))-y[n])*np.array(X1[n])
        w1 = w1 - step_size*derivative
    w = w1[1:]
    b = w1[0]
    assert w.shape == (D,)
    return w, b
...
def binary_predict(X, w, b):
    ...
    """
    for n in range(N):
        preds[n] = 1 if sigmoid(np.matmul(w,np.transpose([X[n]]))+b)>0.5 else 0
...
def multinomial_train(X, y, C, 
    ...
    X1 = np.concatenate((np.transpose([[1 for n in range(N)]]), X1), axis=1)
    w1 = np.copy(w)
    w1 = np.concatenate((np.transpose([[0 for c in range(C)]]), w1), axis=1)
    y1 = np.zeros((N,C))
    for n in range(N):
        y1[n][y[n]] = 1
    for i in range(max_iterations):
        for n in range(N):
            derivative = np.matmul(sigmoid(np.matmul(w1,np.transpose([X1[n]])) - np.transpose([y1[n]])), [X1[n]])
            w1 = w1 - step_size*derivative
    w = w1[:,1:]
    b = np.transpose(w1[:,0:1])[0]
    assert w.shape == (C, D)
    assert b.shape == (C,)
    return w, b
...
def multinomial_predict(X, w, b):
    ...
    """   
    for n in range(N):
        p = (np.matmul(w,np.transpose([X[n]])).T+np.array([b]))[0]
        mult_pred = list(map(sigmoid, p))
        preds[n] = np.argmax(mult_pred)
    assert preds.shape == (N,)
    return preds
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for n in range(N):
        y1[y[n]][n] = 1
    for c in range(C):
        w[c], b[c]=binary_train(X, y1[c], w0=None, b0=None, step_size=0.5, max_iterations=1000)
...
def OVR_predict(X, w, b):
    ...
    """
    for c in range(C):
        pred_c = binary_predict(X, w[c], b[c])
        for n in range(len(pred_c)):
            if pred_c[n] == 1:
                preds[n] = c 
    assert preds.shape == (N,)
    return preds
