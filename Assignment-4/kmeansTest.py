...
#def transform_image(image, code_vectors):
    ...
    N, M = im.shape[:2]
    im = im.reshape(N * M, 3)
    membership=list(map(lambda x_i:np.argmin(list(map(lambda c: np.linalg.norm(x_i-c)**2, code_vectors))),im))
    for i in range(im.shape[0]):
        im[i]=code_vectors[membership[i]]
    return im.reshape(N,M,3)
